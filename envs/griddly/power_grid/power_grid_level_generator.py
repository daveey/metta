...
class PowerGridLevelGenerator():
    ...
    def __init__(self, cfg=None):
        ...
        self.sample_level_wall_density = args_parsing.get_value_possibly_from_range(self.cfg.power_grid_wall_density, np.random.uniform)
        self.sample_num_chargers_per_agent = args_parsing.get_value_possibly_from_range(self.cfg.power_grid_chargers_per_agent, np.random.uniform)
        self.sample_initial_energy = args_parsing.get_value_possibly_from_range(self.cfg.power_grid_initial_energy, np.random.uniform)
    ...
    def _make_level(self):
        ...
        chargers = int(self.num_agents * self.sample_num_chargers_per_agent())
...
def add_env_args(parser: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--power_grid_width",
                   default=[15],
    ...
    p.add_argument("--power_grid_height",
                   default=[15],
                   help='Level height. Can be either a single integer OR a low:high range (e.g., "10:20" to sample from [10, 20) - high excluded)',
                   action=args_parsing.PossiblyNumericRange2Number,
                   str2numeric_cast_fn=int)

    p.add_argument("--power_grid_chargers_per_agent",
                   default=[1],
                   help='Chargers per agent.',
                   action=args_parsing.PossiblyNumericRange2Number,
                   str2numeric_cast_fn=float)

    p.add_argument("--power_grid_initial_energy",
                   default=[100],
                   help='Initial enrergy.',
                   action=args_parsing.PossiblyNumericRange2Number,
                   str2numeric_cast_fn=int)

    p.add_argument("--power_grid_wall_density",
                   default=[0.05],
